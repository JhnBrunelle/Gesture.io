...
def readButtonPress(ser):
	if ser.readline() == 0:
		return False
	else:
		return True

def writeRGB(ser,color):
	ser.write(color)
...
def main():
	...
	ser.close()
	#arduinoCom.writeRGB('b')
	#ser.write(str(3))
	while 1:

		print(len(controller.devices))
		ser = serial.Serial('COM16', 9600)
		print "Button Next"
		while 1:
			if ser.readline()[:-2] == "1":
		...
		# Check to see if the character received was a null character
		if receivedChar == None:
